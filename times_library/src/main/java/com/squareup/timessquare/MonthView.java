...
public class MonthView extends LinearLayout {
    ...
    public static MonthView create(ViewGroup parent, LayoutInflater inflater,
                                   ...
                                   int headerTextColor, Locale locale, WeekCheckListener weekCheckListener) {
    ...
    public static MonthView create(ViewGroup parent, LayoutInflater inflater,
                                   DateFormat weekdayNameFormat, Listener listener, final Calendar today, int dividerColor,
                                   int dayBackgroundResId, int dayTextColorResId, int titleTextColor, boolean displayHeader,
                                   int headerTextColor, List<CalendarCellDecorator> decorators, Locale locale, final WeekCheckListener weekCheckListener) {
        ...
        for (int offset = 0; offset < 7; offset++) {
            ...
            final CheckBox checkBox = (CheckBox) headerRow.getChildAt(offset);
//      checkBox.setText(weekdayNameFormat.format(today.getTime()));
            checkBox.setText(getWeek(today.getTime()));
            final int finalOffset = offset;
            checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    // 点击时偏移到对应星期几
                    today.set(Calendar.DAY_OF_WEEK, getDayOfWeek(firstDayOfWeek, finalOffset, view.isRtl));

                    if (isChecked) {
                        weekCheckListener.onCheck(today.get(Calendar.DAY_OF_WEEK));
                    } else {
                        weekCheckListener.onUnCheck(today.get(Calendar.DAY_OF_WEEK));
                    }
    ...
    public void setDesc(Date date, String desc){
    ...
    public void checkWeek(int week) {
        ...
        for (int offset = 0; offset < 7; offset++) {
            CheckBox checkBox = (CheckBox) headerRow.getChildAt(offset);
            if (offset + 1 == week) {
    ...
    public void checkWeek(Set<Integer> selectedWeekSet) {
        if (selectedWeekSet == null) return;

        Iterator<Integer> iterator = selectedWeekSet.iterator();
        while (iterator.hasNext()) {
            checkWeek(iterator.next());
        }
    }
    ...
    public interface WeekCheckListener {
        void onCheck(int week);

        void onUnCheck(int week);
    }
