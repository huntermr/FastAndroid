package demo.ui.widget.pulltozoomview;
...
public abstract class PullToZoomBase<T extends View> extends LinearLayout implements IPullToZoom<T> {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            //初始化状态View
            TypedArray a = getContext().obtainStyledAttributes(attrs, com.ecloud.pulltozoomview.R.styleable.PullToZoomView);

            int zoomViewResId = a.getResourceId(com.ecloud.pulltozoomview.R.styleable.PullToZoomView_zoomView, 0);
            if (zoomViewResId > 0) {
                mZoomView = mLayoutInflater.inflate(zoomViewResId, null, false);
            }

            int headerViewResId = a.getResourceId(com.ecloud.pulltozoomview.R.styleable.PullToZoomView_headerView, 0);
            if (headerViewResId > 0) {
                mHeaderView = mLayoutInflater.inflate(headerViewResId, null, false);
            }

            isParallax = a.getBoolean(com.ecloud.pulltozoomview.R.styleable.PullToZoomView_isHeaderParallax, true);
